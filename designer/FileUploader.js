...
function uploaderDesignId() {
    return 'uploadfile_' + Math.random().toString(36).substring(3, 3) + Number(("" + (Date.now()))).toString(36);
}

function FileUploader(container, options, designTime) {
    var
        isDesignTime, fileType, $template, settings = {},
        pcontainer, baseFiles = null,
        elements = {},
    ...
    Object.defineProperty(this, 'baseFiles', {
        get: function () {
            return baseFiles;
        },
        set: function (value) {
...
FileUploader.prototype._buildHtml = function () {
    ...
    this.settings.attrs.class = 'custom-file-input';
    ...
    var disabled = (this.isDesignTime) ? ' disabled ' : '';
    ...
    <div class="col-auto mt-3 d-none" id='` + this.getId() + `-c-remove'><button remove="true" class="btn btn-danger btn-sm rounded-circle"><i class="fas fa-trash"></i></button></div>
    </div></label>`);
...
FileUploader.prototype._bindEvents = function () {
    this.elements.input.on({
        change: (ev) => {
            this.removeFile();
            this.baseFiles = ev.target.files;
            // resetError();
...
FileUploader.prototype.readURL = function (input) {
    if (input.files && input.files[0]) {
        ...
        /*   var reader = new FileReader();
          reader.onload = (e) => {
              ...
              this.fileType = data.substr(0, data.indexOf(';')).slice(5).split("/");
              console.info(this.fileType)
              var isImage = (this.fileType[0] == "image") ? true : false;
              this.displayProviderLogo(e.target.result, isImage);
              //handler(e);
          }
