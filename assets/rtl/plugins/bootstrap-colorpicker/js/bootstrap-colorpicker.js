...
exports.default = {
   ...
   * if false, the invalid color will be right in the input,
...
var SliderHandler = function () {
  ...
  _createClass(SliderHandler, [{
    key: 'defaultOnMove',
    value: function defaultOnMove(top, right) {
      ...
      slider.guideStyle.right = right + 'px';
      slider.guideStyle.top = top + 'px';

      // Adjust the color
      if (slider.callRight) {
        color[slider.callRight](right / slider.maxRight);
      }
      if (slider.callTop) {
    ...
    value: function moved(e) {
      ...
      var right = Math.max(0, Math.min(this.currentSlider.maxRight, this.currentSlider.right + ((e.pageX || this.mousePointer.right) - this.mousePointer.right)));

      var top = Math.max(0, Math.min(this.currentSlider.maxTop, this.currentSlider.top + ((e.pageY || this.mousePointer.top) - this.mousePointer.top)));

      this.onMove(top, right);
    }
...
var PickerHandler = function () {
  ...
  _createClass(PickerHandler, [{
    ...
    value: function update() {
      ...
      // Set guides position
      if (hueGuide.length) {
        hueGuide.css(vertical ? 'top' : 'right', (vertical ? slider.hue.maxTop : slider.hue.maxRight) * (1 - hsva.h));
      }
      if (alphaGuide.length) {
        alphaGuide.css(vertical ? 'top' : 'right', (vertical ? slider.alpha.maxTop : slider.alpha.maxRight) * (1 - hsva.a));
      }
      if (saturationGuide.length) {
        saturationGuide.css({
          'top': slider.saturation.maxTop - hsva.v * slider.saturation.maxTop,
          'right': hsva.s * slider.saturation.maxRight
        });
      }
      ...
      if (this.options.horizontal) {
        alphaBg = 'linear-gradient(to left, ' + hexColor + ' 0%, transparent 100%)';
      } else {
