...
<!DOCTYPE html>
<html lang="ar" dir="<?= $PageDirection ?>">
...
<head>
    ...
    <style>
        .margin-table td:not(.m-center) {
            padding: 2px !important;
        }
        .margin-table td.m-center {
            padding:0px 15px 0px 15px !important;
        }
        .margin-table .control-xs {
            display: block;
            width: 100%;
            line-height: 1;
            color: #212529;
...
<body class="hold-transition sidebar-mini layout-fixed  sidebar-collapse" dir="<?= $PageDirection ?>">
    ...
    <div class="wrapper">
        ...
        <footer class="main-footer text-center"> fdfdf </footer>
    </div>
    ...
    <script>
        $(function() {
            ...
            /*  $.getJSON('RecoredDesign/form.json', function(data) {
            ...
            $.ajax({
                ...
                complete: function(json) {
                    try {
                        ...
                        /* JSON.parse(json.responseText,(key,value)=>{
                            if(key==='FormJson')
                            value = jQuery.parseJSON(value);
                            data[key]=value;
                            console.info(key, value)
                            console.info('isArray', Array.isArray(value) )
                        }) */


                        var data = jQuery.parseJSON(json.responseText);
                        data.FormJson = jQuery.parseJSON(data.FormJson);

                        if (data.FormJson.length > 0) {
                          
                            getJsonArrayHtml($('#toolsContainer'), data.FormJson);
                        } else {
                            if (data.msg) {
